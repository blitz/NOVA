...
class Vmcs
{
    public:
        ...
        static uint64 ctrl_pin      CPULOCAL;
        static uint64 ctrl_cpu      CPULOCAL;
        static uint64 ctrl_exi      CPULOCAL;
        static uint64 ctrl_ent      CPULOCAL;
        static uint32 revision      CPULOCAL;
        static mword  cr0_fix0      CPULOCAL;
        static mword  cr0_fix1      CPULOCAL;
        static mword  cr4_fix0      CPULOCAL;
        static mword  cr4_fix1      CPULOCAL;
        ...
        enum Encoding
        {
            ...
            // 64-Bit Host State
            PERF_GLOBAL_CTRL        = 0x2c04ul,

            // 32-Bit Control Fields
            PIN_EXEC_CTRL           = 0x4000ul,
            CPU_EXEC_CTRL_1         = 0x4002ul,
            EXC_BITMAP              = 0x4004ul,
        ...
        Vmcs() : rev (revision)
        {
            ...
            bool ret;
            asm volatile ("vmxon %1; seta %0" : "=q" (ret) : "m" (phys));
            assert (ret);
        }
        ...
        inline void clear()
        {
            ...
            bool ret;
            asm volatile ("vmclear %1; seta %0" : "=q" (ret) : "m" (phys));
            assert (ret);
        }
        ...
        inline void make_current()
        {
            ...
            bool ret;
            asm volatile ("vmptrld %1; seta %0" : "=q" (ret) : "m" (phys));
            assert (ret);
        }
        ...
        {
            mword val;
            asm volatile ("vmread %1, %0" : "=rm" (val) : "r" (enc));
            return val;
        }
        ...
        {
            asm volatile ("vmwrite %0, %1" : : "rm" (val), "r" (enc));
        }
