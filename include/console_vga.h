...
class Console_vga : public Console
{
    private:
        ...
        {
            memset (reinterpret_cast<void *>(VGACN_SADDR), 0, 160 * _num_row);
        }

        ALWAYS_INLINE
        inline void clear_row (unsigned row)
        {
            memcpy (reinterpret_cast<void *>(VGACN_SADDR), reinterpret_cast<void *>(VGACN_SADDR + 160), 160 * row);
            memset (reinterpret_cast<void *>(VGACN_SADDR + 160 * row), 0, 160);
        }
    ...
    public:
        ...
        Console_vga() : _num_row (24), _row (0), _col (0) {}

        INIT
        void init();

        ALWAYS_INLINE
        inline void put (unsigned row, unsigned col, Color color, int c)
        {
            *reinterpret_cast<unsigned short volatile *>(VGACN_SADDR + row * 160 + col * 2) = static_cast<unsigned short>(color << 8 | c);
        }
