...
void Console::print_number (uint64 val, unsigned base, unsigned width, unsigned flags)
{
    ...
    unsigned count = buffer + sizeof buffer - ptr;

    unsigned n = count + (flags & FLAG_ALT_FORM ? 2 : 0);
...
const char *Console::print_format (char const *format, va_list& args)
{
...
void Console::vprintf (char const *format, va_list args)
{
    ...
    for (;;) {
        switch (*format) {
            ...
            case '%':
                format = print_format (format, args);
                break;
