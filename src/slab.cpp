      ...
      head  (0)
{
    ...
    for (unsigned i = avail; i; i--, link -= cache->buff) {
...
Slab_cache::Slab_cache (char const *name, size_t elem_size, unsigned elem_align)
          : curr (0),
            head (0),
...
{
    trace (TRACE_MEMORY, "Slab Cache:%p (O:%s S:%u A:%u)",
           this,
           name,
           elem_size,
           elem_align);
